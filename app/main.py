...
# Instrument the FastAPI application before applying the outer CORS wrapper.
# The CORS wrapper is added at the absolute end of this file so that CORS
# headers are also attached to responses generated by middleware and global
# exception handlers.
metrics.instrument(app)
...
@app.post("/api/auth/token")
def login(req: LoginRequest):
    """Authenticate a user and return the login token response."""
    print("===== LOGIN ENDPOINT CALLED =====")
    result = authenticate_user(req)
    print("Login result generated successfully")
    return result
...
# =============================================================================
# IMPORTANT:
# Keep this block at the absolute end of the file, after every @app route,
# startup handler, WebSocket endpoint and exception handler.
#
# The browser sends an OPTIONS preflight request before JSON POST requests such
# as /api/auth/token. Wrapping the complete application ensures that normal
# responses and error responses both include the required CORS headers.
...
app = CORSMiddleware(
    ...
    # Supports Netlify deploy-preview and branch-deploy URLs, for example:
    # https://deploy-preview-12--iridescent-lollipop-e693d7.netlify.app
    # https://feature-login--iridescent-lollipop-e693d7.netlify.app
    allow_origin_regex=(
        r"^https://(?:[a-zA-Z0-9-]+--)?"
        r"iridescent-lollipop-e693d7\.netlify\.app$"
    ),

    # Explicit origins are used, so credentialed requests can also be accepted
    # if the frontend later sends cookies or uses credentials: "include".
    allow_credentials=True,

    # Allow all HTTP methods used by the API, including OPTIONS preflight.
    allow_methods=["*"],

    # Allow Content-Type, Authorization, X-Tenant and any future custom headers.
    allow_headers=["*"],

    # Allow the frontend to read download-related response headers.
    expose_headers=[
        "Content-Disposition",
        "Content-Length",
    ],

    # Cache successful preflight responses for one day.
    max_age=86400,
)
